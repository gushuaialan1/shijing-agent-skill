...
**输入（Input）**
> 人物：苏轼 | 文本：《定风波》| 受众：25-35 岁职场人 | 困境：失业/辉南压力

**输出（Output 摘要）**
> 硬事实卡 → 微场景（沙湖道中遇雨） → 词眼卡（莫听、何妨、轻胜马、谁怕、任平生） → 古今对照卡 → 1200 字成稿
...
| 提示词模板 | Markdown 模板 | 结构化 LLM Prompt |
| 辅助工具 | Python 3.10+ | CLI 脚本，支持 JSON/YAML 配置 |
...
```bash
pip install -e .
shijing-agent --config task.yaml
```
...
1. 阅读 `references/` 下的研究文档
2. 修改 `SKILL.md` 或 `templates/prompt.md`
3. 本地验证：将技能复制到 `~/.hermes/skills/shijing-agent-skill`，重新加载 Hermes
4. 运行 `pytest tests/` 检查脚本正确性
...
- [PROJECT_BRIEF.md](PROJECT_BRIEF.md) — 项目落地规划
